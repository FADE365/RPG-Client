...
import com.example.examplemod.Module.AUTODUPE.PorableDupe.modules.CommandsModule;
import com.example.examplemod.Module.AUTODUPE.PorableDupe.modules.PacketInterceptionModule;
...
import static com.example.examplemod.Module.AUTODUPE.PorableDupe.FamilyFunPack.*;
...
public class AdCommand extends Command {
	...
	private void dupe() {
		...
		if(mc.player.isRiding() && mc.world != null && enabled) {
			...
			sleep(delay);
			if (!mc.world.getChunkFromBlockCoords(new BlockPos(donkx, donky, donkz)).isEmpty()) {
